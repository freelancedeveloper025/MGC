...
from test_framework.mininode import mgchash
from test_framework.util import (assert_equal,
                                 bytes_to_hex_str,
                                 hash256,
                                 )

def mgchash_helper(b):
    return encode(mgchash(b)[::-1], 'hex_codec').decode('ascii')
...
class ZMQTest (BitcoinTestFramework):
    ...
    def _zmq_test(self):
        ...
        for x in range(num_blocks):
            ...
            assert_equal(genhashes[x], mgchash_helper(block[:80]))
