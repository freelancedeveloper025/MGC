...
const char * const BITCOIN_CONF_FILENAME = "mgc.conf";
const char * const BITCOIN_PID_FILENAME = "mgcd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "mgcd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MgcCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MgcCore
    // Mac: ~/Library/Application Support/MgcCore
    // Unix: ~/.mgccore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MgcCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MgcCore";
#else
    // Unix
    return pathRet / ".mgccore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty mgc.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Mgc Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Mgc Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2014, nEndYear) + "The Mgc Core developers";
    }
